...
# Ajout de la colonne "email" dans USERS et DEMANDES pour l'envoi auto
...
if "Yael" not in udb_init["pseudo"].values:
    admin_row = pd.DataFrame([["yael@admin.com", "Yael", "Yassine05", "Admin"]], columns=["email", "pseudo", "password", "role"])
    ...
    if st.session_state["view"] == "login":
        ...
        with c3:
            if st.button(L["btn_forgot"]):
                ...
                else: st.warning("Entre ton pseudo pour l'oubli.")
        ...
        if st.button("Envoyer la demande"):
            if np == ncp and nu and ne:
                ...
                st.success("Demande enregistrée !"); st.session_state["view"] = "login"
            else: st.error("Vérifie les champs et l'email.")
...
# --- 9. NOTIFICATIONS (GESTION YAEL AVEC EMAIL AUTO) ---
if st.session_state["page_params"] == "notif" and st.session_state["is_admin"]:
    st.title("🔔 Notifications & Validation")
    # Inscriptions
    st.subheader("Nouveaux Membres")
    ddb = pd.read_csv(DEMANDES_FILE)
    for i, r in ddb.iterrows():
        c1, c2, c3 = st.columns([2,1,1])
        c1.write(f"Insc : **{r['pseudo']}** ({r['email']})")
        if c2.button("✅", key=f"ok_{i}"):
            new_u = pd.DataFrame([[r['email'], r['pseudo'], r['password'], "Membre"]], columns=["email", "pseudo", "password", "role"])
            ...
            # Envoi Email auto
            envoyer_email(r['email'], "Compte Bilan Coran Validé !", f"Salam {r['pseudo']},\n\nYael a validé ton accès. Tu peux te connecter !")
            ddb.drop(i).to_csv(DEMANDES_FILE, index=False); st.rerun()
        if c3.button("❌", key=f"no_{i}"):
            ddb.drop(i).to_csv(DEMANDES_FILE, index=False); st.rerun()
    
    # Mdp oubliés
    st.subheader("Mots de passe perdus")
    fdb = pd.read_csv(FORGOT_FILE)
    udb = pd.read_csv(USERS_FILE)
        ...
        if not u_info.empty:
            ...
            if c3.button("💾", key=f"fsave_{i}"):
                if n_mdp:
                    ...
                    # Envoi Email auto
                    envoyer_email(u_info.iloc[0]['email'], "Nouveau mot de passe", f"Salam,\n\nTon nouveau mot de passe est : {n_mdp}")
