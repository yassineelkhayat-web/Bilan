...
if "Yael" not in udb["pseudo"].values:
    ...
    pd.concat([udb, admin_row], ignore_index=True).to_csv(USERS_FILE, index=False)
...
TRAD = {
    "Français": {
        ...
        "etat": "📊 État actuel", "col_prog": "Progression", "exp_msg": "💬 WhatsApp", "echeance": "Échéance :", 
        "copier": "Copier :", "exp_maj": "📝 Mise à jour", "pers": "Personne :", "pg_act": "Page actuelle :", 
        "rythme": "Rythme :", "btn_save": "💾 Enregistrer", "exp_prec": "🔄 Date précise", "date_prec": "Date :",
        ...
        "hadith_btn": "GÉNÉRER MESSAGE HADITH", "khatma": "Objectif Khatmas", "home_btn": "🏠 Accueil", 
        "view_prog": "📊 Progression", "notif": "🔔 Notifications", "avant_ram": "J-{} avant Ramadan", 
        "pendant_ram": "J-{} avant la fin", "date_deb": "Début :", "date_fin": "Fin :", "info_title": "ℹ️ Infos"
    },
    "العربية": {
        ...
        "etat": "📊 الحالة الراهنة", "col_prog": "التقدم", "exp_msg": "💬 واتساب", "echeance": "الموعد :", 
        "copier": "نسخ :", "exp_maj": "📝 تحديث", "pers": "الشخص :", "pg_act": "الصفحة الحالية :", 
        "rythme": "المعدل :", "btn_save": "💾 حفظ", "exp_prec": "🔄 تاريخ دقيق", "date_prec": "التاريخ :",
        ...
        "view_prog": "📊 التقدم", "notif": "🔔 تنبيهات", "avant_ram": "متبقي {} أيام", 
        "pendant_ram": "متبقي {} أيام", "date_deb": "البداية :", "date_fin": "النهاية :", "info_title": "ℹ️ معلومات"
    }
}
...
st.markdown(f"<style>h1,h2,h3,p,label,span{{color:{COLOR}!important;}} div.stButton>button{{border:2px solid {COLOR}!important; color:{COLOR}!important; border-radius:10px; font-weight:bold;}} div.stButton>button:hover{{background-color:{COLOR}!important; color:white!important;}}</style>", unsafe_allow_html=True)
    ...
    if st.session_state["view"] == "login":
        ...
        with c3:
            if st.button(L["btn_forgot"]):
                ...
                else: st.warning("Entre ton pseudo d'abord.")
    elif st.session_state["view"] == "signup":
        st.title("📝 Inscription")
        nu, np, ncp = st.text_input("Pseudo"), st.text_input("Mdp", type="password"), st.text_input("Confirm", type="password")
...
with st.sidebar:
    ...
    if st.button(L["home_btn"], use_container_width=True): st.session_state["page_params"] = False; st.rerun()
    if st.button(L["params"], use_container_width=True): st.session_state["page_params"] = "settings"; st.rerun()
    if st.session_state["is_admin"]:
        nb_n = len(pd.read_csv(DEMANDES_FILE)) + len(pd.read_csv(FORGOT_FILE))
        if st.button(f"{L['notif']} ({nb_n})", use_container_width=True): st.session_state["page_params"] = "notif"; st.rerun()
    st.divider()
    if st.button(L["mode_norm_btn"] if st.session_state["ramadan_mode"] else L["mode_ram_btn"], use_container_width=True):
        st.session_state["ramadan_mode"] = not st.session_state["ramadan_mode"]; st.rerun()
    if st.button(L["btn_logout"], use_container_width=True): st.session_state["auth"] = False; st.rerun()

# --- 8. PAGES SPECIALES ---
if st.session_state["page_params"] == "notif" and st.session_state["is_admin"]:
    st.title("🔔 Centre de Gestion")
    
    # --- SECTION INSCRIPTIONS ---
    st.subheader("Nouveaux Comptes")
    ddb = pd.read_csv(DEMANDES_FILE)
    for i, r in ddb.iterrows():
        c1, c2, c3 = st.columns([2,1,1])
        c1.write(f"**{r['pseudo']}** veut s'inscrire.")
        if c2.button("✅ Approuver", key=f"ok_{i}"):
    ...
    # --- SECTION MOT DE PASSE OUBLIÉ (TA DEMANDE) ---
    st.subheader("Mots de passe oubliés")
    fdb = pd.read_csv(FORGOT_FILE)
    udb = pd.read_csv(USERS_FILE)
    
    for i, r in fdb.iterrows():
        pseudo_perdu = r['pseudo']
        # On cherche son ancien MDP dans la base
        user_info = udb[udb["pseudo"] == pseudo_perdu]
        
        with st.container():
            col_info, col_action = st.columns([2,2])
            if not user_info.empty:
                ancien_mdp = user_info.iloc[0]['password']
                col_info.write(f"Utilisateur : **{pseudo_perdu}**")
                col_info.info(f"Ancien MDP : `{ancien_mdp}`")
                
                # Input pour changer le MDP
                nouveau_mdp = col_action.text_input("Nouveau MDP", key=f"new_val_{i}")
                if col_action.button("💾 Mettre à jour", key=f"btn_up_{i}"):
                    if nouveau_mdp:
                        # Mise à jour dans le fichier USERS
                        udb.loc[udb["pseudo"] == pseudo_perdu, "password"] = nouveau_mdp
                        udb.to_csv(USERS_FILE, index=False)
                        # Supprimer la notification
                        fdb.drop(i).to_csv(FORGOT_FILE, index=False)
                        st.success(f"MDP de {pseudo_perdu} modifié !")
                        st.rerun()
            else:
                col_info.warning(f"Pseudo '{pseudo_perdu}' introuvable dans la base.")
                if col_action.button("Effacer l'alerte", key=f"del_err_{i}"):
...
# Filtre : Admin voit tout, Membre voit son profil
    ...
    with c2:
        with st.expander(L["exp_msg"]):
            ...
            for n, r in view_df.iterrows():
                p = (int(r["Page Actuelle"]) + int(r["Rythme"])) % 604 or 1
                msg += f"• *{n}* : p.{p}\n"
            st.text_area(L["copier"], msg)

    st.subheader(L["plan"])
    plan_df = pd.DataFrame(index=[(auj + timedelta(days=i)).strftime("%d/%m") for i in range(15)])
    for n, r in view_df.iterrows():
        plan_df[n] = [int((int(r["Page Actuelle"]) + (int(r["Rythme"]) * i)) % 604 or 1) for i in range(15)]
    st.dataframe(plan_df, use_container_width=True)
else:
    st.info("Aucun profil associé à ce compte. Contacte Yael.")
