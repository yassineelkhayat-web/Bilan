...
import os
import random
import requests # Ajouté pour les notifications
...
def envoyer_confirmation_utilisateur(pseudo, email_dest):
    ...
    payload = {
        ...
        "template_params": {
            "to_name": pseudo,
            "user_email": email_dest, # Doit être {{user_email}} dans ton template EmailJS
...
# On ajoute "user_email" dans les colonnes pour l'automatisation
...
if st.session_state["page"] == "admin":
    ...
    for i, r in ddb.iterrows():
        ...
        c1.write(f"Inscrire **{r['pseudo']}** ({r['user_email']}) ?")
        if c2.button("✅", key=f"a_{i}"):
            ...
            # --- ENVOI MAIL CONFIRMATION AUTOMATIQUE ---
